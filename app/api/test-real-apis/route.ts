...
export async function GET() {
  try {
    ...
    // Test the actual domains API logic (without authentication check)
    ...
    // Test the actual domains API enrichment logic
    const enrichedDomains = await Promise.all((domains || []).map(async (domain: any) => {
      ...
      if (categoryError) {
        console.error(`❌ [TEST] Error fetching category data for ${domain.domain}:`, categoryError)
      }
      
      if (!categoryData || categoryData.length === 0) {
        console.warn(`⚠️ [TEST] No categorization data found for domain: ${domain.domain}`)
        return {
          ...domain,
    ...
    // Test the actual URLs API logic
    ...
    // Test the actual URLs API enrichment logic
    const enrichedUrls = await Promise.all((urls || []).slice(0, 3).map(async (urlData: any) => {
      ...
      for (const urlVariation of urlVariations) {
        ...
        if (normalizedError) {
          console.error(`❌ [TEST] Normalized query error for ${urlVariation}:`, normalizedError)
        }
        ...
        if (exactError) {
          console.error(`❌ [TEST] Exact query error for ${urlVariation}:`, exactError)
        }
    ...
    return NextResponse.json({
      success: true,
      data: {
        domains: enrichedDomains.filter(d => d.domain === 'qovery.com'),
        urls: enrichedUrls
      }
    })
