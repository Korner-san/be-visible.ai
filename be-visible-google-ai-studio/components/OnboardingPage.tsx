...
const CompetitorLogo = ({ name, domain }: { name: string; domain?: string }) => {
  ...
  const resolvedDomain = domain && domain.trim()
    ? domain.trim()
    : (name.includes('.') && !name.includes(' ') ? name : name.toLowerCase().replace(/\s/g, '') + '.com');
...
export const OnboardingPage: React.FC<OnboardingPageProps> = ({ existingBrandId, onComplete, onNavigate }) => {
    ...
    const loadExisting = async () => {
      ...
      if (brand?.onboarding_answers) {
        ...
        setData(prev => ({
          ...
          competitors: a.competitors?.length
            ? a.competitors.map((c: any) => typeof c === 'string' ? { name: c, domain: '' } : c)
            : prev.competitors,
      ...
      // Include 'improved' — prompts stay in that status until complete-final assigns waves
          ...
          setData(prev => ({
            ...
            competitors: bd.competitors?.length
              ? [
                  ...
                  ...Array(Math.max(0, 6 - bd.competitors.slice(0, 6).length)).fill({ name: '', domain: '' }),
                ]
              : prev.competitors,
  ...
  return (
    <div className="min-h-screen overflow-y-auto bg-gradient-to-br from-slate-50 to-blue-50 flex items-start justify-center py-6 px-4">
      <div className="w-full max-w-3xl onboarding-container">
        <div className="bg-white rounded-[28px] border border-[#E2E8F0] shadow-2xl flex flex-col">
          <div className="p-8">
            {/* Header */}
