...
/**
 * Check if all batches are complete for a given brand and date
 */
async function checkAllBatchesComplete(brandId, reportDate) {
  ...
  // All schedules must be 'completed'
  const allComplete = schedules.every(s => s.status === 'completed');

  console.log(`📊 Batch status: ${schedules.filter(s => s.status === 'completed').length}/${schedules.length} complete`);
