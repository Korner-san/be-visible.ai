-- Improve URL matching logic in enhanced citation functions

-- Drop and recreate get_enhanced_citation_urls_by_domain with better URL matching
...
BEGIN
  ...
  url_categorization AS (
    SELECT 
      ...
      COALESCE(
        -- Direct match
        (SELECT ucf.content_structure_category 
         FROM url_inventory ui 
         JOIN url_content_facts ucf ON ui.id = ucf.url_id 
         WHERE ui.url = us.url OR ui.normalized_url = us.url),
        -- www prefix variations
        (SELECT ucf.content_structure_category 
         FROM url_inventory ui 
         JOIN url_content_facts ucf ON ui.id = ucf.url_id 
         WHERE (us.url LIKE 'https://%' AND ui.url = REPLACE(us.url, 'https://', 'https://www.')) OR
        ...
        -- Domain-level fallback (most common category for this domain)
        (SELECT ucf.content_structure_category
         FROM url_inventory ui
         JOIN url_content_facts ucf ON ui.id = ucf.url_id
         WHERE ui.domain = p_domain AND ucf.content_structure_category IS NOT NULL
         GROUP BY ucf.content_structure_category
         ORDER BY COUNT(*) DESC
         LIMIT 1)
      ) as content_structure_category
    FROM url_stats us
  )
  SELECT 
    uc.url,
    uc.times_cited,
    uc.distinct_ai_responses,
    uc.prompt_coverage,
    uc.model_coverage,
    uc.first_seen_at,
    uc.last_seen_at,
    uc.content_structure_category
  FROM url_categorization uc
  ORDER BY uc.times_cited DESC, uc.last_seen_at DESC;
END;
$$;
