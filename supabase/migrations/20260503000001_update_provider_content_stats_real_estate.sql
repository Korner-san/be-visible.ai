-- Keep provider-filtered content type stats on the same brand-specific taxonomy
-- path as the default content stats RPC.

DROP FUNCTION IF EXISTS public.get_content_type_stats(UUID, DATE, DATE);
DROP FUNCTION IF EXISTS public.get_content_type_stats(UUID, DATE, DATE, TEXT);
...
BEGIN
  ...
  brand_prompt_results AS (
    ...
    WHERE pr.daily_report_id IN (SELECT id FROM brand_reports)
      ...
      AND (p_provider IS NULL OR pr.provider = p_provider)
  ),
  brand_citations AS (
    SELECT
      uc.url_id,
...
COMMENT ON FUNCTION public.get_content_type_stats(UUID, DATE, DATE, TEXT)
IS 'Returns provider-filtered content structure stats. Uses brand-specific overrides for real_estate_israel brands and global URL classifications for general brands.';
