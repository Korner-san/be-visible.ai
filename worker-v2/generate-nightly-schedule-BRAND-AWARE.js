...
// Configuration
const MAX_PROMPTS_PER_USER = 50; // V2: 50 prompts per brand (5 topics × 10 prompts)
...
async function createScheduleRecords(prompts) {
  ...
  // Step 0: Prompt-aware dedup — remove already-scheduled prompts before batching.
  // Replaces the old binary brand-level check ("any row exists → skip brand") which
  // prevented partial re-scheduling after cross-brand cascade deletions.
  // Groups prompts by brand, queries ALL existing schedule rows for each brand+date,
  // and filters out only the prompts already covered. Missing prompts are re-added.
  const promptsByBrand = {};
  for (const p of prompts) {
    if (!promptsByBrand[p.brandId]) promptsByBrand[p.brandId] = [];
    promptsByBrand[p.brandId].push(p);
  }

  const filteredPrompts = [];
  for (const [brandId, brandPrompts] of Object.entries(promptsByBrand)) {
    const scheduleDate = getUserTomorrowDate(brandPrompts[0].userTimezone || 'UTC');
    // Fetch ALL existing rows for this brand+date (not just limit 1) so we can
    // collect every already-scheduled prompt ID across all batches.
    ...
    const scheduledSet = new Set(
      (existingRows || []).flatMap(row => row.prompt_ids || [])
    );

    if (scheduledSet.size === 0) {
      filteredPrompts.push(...brandPrompts);
      continue;
    }

    const missing = brandPrompts.filter(p => !scheduledSet.has(p.promptId));
    const alreadyCount = brandPrompts.length - missing.length;

    if (missing.length === 0) {
      console.log(`   ⚠️  Brand ${brandId.substring(0, 8)}: all ${brandPrompts.length} prompts already scheduled for ${scheduleDate} — skipping`);
    } else {
      console.log(`   ℹ️  Brand ${brandId.substring(0, 8)}: ${alreadyCount} already scheduled, re-adding ${missing.length} missing for ${scheduleDate}`);
      filteredPrompts.push(...missing);
    }
  }

  if (filteredPrompts.length === 0) {
    console.log('   All brands fully scheduled — nothing to add.\n');
    return [];
  }

  const promptsToSchedule = filteredPrompts;

  // Step 1: Interleave brands to prevent clustering
  const interleaved = interleaveBrandPrompts(promptsToSchedule);
  ...
  console.log(`   Created ${batches.length} mixed batches from ${promptsToSchedule.length} prompts`);
  ...
  // Step 5: Create schedule records
  // (Dedup now handled at prompt level in Step 0 — no brand-level skipping needed here)
