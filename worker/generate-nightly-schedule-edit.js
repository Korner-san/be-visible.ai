...
async function getEligibleUsers() {
  const { data: users, error } = await supabase
    ...
    .in('subscription_plan', ['starter', 'pro', 'custom'])
...
function generateRandomTimeSlots(numSlots, minHour, maxHour, minSpacingMinutes) {
  ...
  while (slots.length < numSlots) {
    ...
    if (usedMinutes.size > totalMinutes / minSpacingMinutes + 100) {
      throw new Error('Unable to generate non-overlapping time slots');
    }
  }
